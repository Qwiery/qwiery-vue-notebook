<template>
	...
	<div class="w-full bg-primary dark:bg-[#333] flex">
		<div v-if="!dashboardMode" class="flex text-white/70 cursor-pointer m-2" @click="addCode()">
			<icon-add class="stroke-[#fff]   mt-1"></icon-add>
			<span class="ml-1">Code</span></div>
		<div v-if="!dashboardMode" class="flex text-white/70 cursor-pointer m-2" @click="addMarkdown()">
			<icon-add class="stroke-[#fff]   mt-1"></icon-add>
			<span class="ml-1">Markdown</span></div>
...
<script setup lang="ts">
	import { Message, MarkdownMessage, CodeMessage, ErrorMessage } from "@orbifold/entities";
	...
	onMounted(() => {
		...
		// notebook.setInterpreter("js", async () => {
		// 	return ErrorMessage.fromString("Tadaa");
		// });
	});
